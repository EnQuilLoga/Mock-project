    ...
    <div className='mt-12 flex justify-center'>
        <form className='w-full max-w-xl ' onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="w-full md:w-auto px-3 mb-6 md:mb-0">
                ...
                <input className='shadow appearance-none border rounded-2xl w-full py-2.5 px-3 mb-1 text-gray-700 leading-tight focus:outline-none' type="text" id="firstName" placeholder='First Name' 
                {...register("name.firstname", {
                    required: true,
                    maxLength: 20,
                    pattern: /^[A-Za-z]+$/i
                })}/> 
                {errors?.name?.firstname?.type === "required" && <p className='text-red-500'>First name is required</p>}
                {errors?.name?.firstname?.type === "pattern" && (<p className='text-red-500'>Alphabetical characters only</p>)}

                {/* Lastname */}
                <label className='block uppercase text-sm font-bold text-gray-700 my-2 tracking-wide'>Last Name</label>
                <input className='shadow appearance-none border rounded-2xl w-full py-2.5 px-3 mb-1 text-gray-700 leading-tight focus:outline-none' type="text" id="lastName" placeholder='Last Name'
                {...register("name.lastname", {
                    required: true,
                    maxLength: 20,
                    pattern: /^[A-Za-z]+$/i
                })} />
                {errors?.name?.lastname?.type === "required" && <p className='text-red-500'>Last name is required</p>}
                {errors?.name?.lastname?.type === "pattern" && (<p className='text-red-500'>Alphabetical characters only</p>)}
                

                {/* Email  */}
                <label className='block uppercase text-sm font-bold text-gray-700 my-2 tracking-wide'>Email</label>
                <input className='shadow appearance-none border rounded-2xl w-full py-2.5 px-3 mb-1 text-gray-700 leading-tight focus:outline-none' type="email" id="email" placeholder='example@gmail.com'
                {...register("email", {
                    required: true,
